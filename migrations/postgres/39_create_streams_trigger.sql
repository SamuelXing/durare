...
CREATE OR REPLACE FUNCTION streams_function() RETURNS TRIGGER AS $$
DECLARE
...
ALTER FUNCTION streams_function() SET search_path = pg_catalog, pg_temp;

DROP TRIGGER IF EXISTS dbos_streams_trigger ON streams;
CREATE TRIGGER dbos_streams_trigger
AFTER INSERT ON streams
FOR EACH ROW EXECUTE FUNCTION streams_function();
