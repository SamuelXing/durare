...
DROP FUNCTION IF EXISTS enqueue_workflow(
...
-- Pin search_path on the new enqueue_workflow overload, extending the
-- hardening from migration 20. Unlike the reference SQL (which schema-qualifies
-- every table reference), this port's function bodies use unqualified names
-- resolved via search_path — so the pin must include the system schema itself,
-- captured at migration time via current_schema(). Pinning only
-- pg_catalog, pg_temp would leave the function unable to see its own tables.
DO $do$
BEGIN
    EXECUTE format(
        'ALTER FUNCTION enqueue_workflow(
             TEXT, TEXT, JSON[], JSON, TEXT, TEXT, TEXT, TEXT, BIGINT, BIGINT,
             TEXT, INT4, TEXT, TEXT, TEXT, BIGINT
         ) SET search_path = %I, pg_catalog, pg_temp',
        current_schema());

    -- Repair send_message, which migration 20 pinned to pg_catalog, pg_temp
    -- alone: with unqualified table references that pin makes every call fail
    -- with 'relation "notifications" does not exist'. Re-pin it the same
    -- schema-aware way.
    EXECUTE format(
        'ALTER FUNCTION send_message(TEXT, JSON, TEXT, TEXT)
         SET search_path = %I, pg_catalog, pg_temp',
        current_schema());
END
$do$;
